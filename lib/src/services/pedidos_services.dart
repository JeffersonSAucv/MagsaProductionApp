...
final _api = 'https://magsa-sistems-db-heroku.herokuapp.com';
...
class PedidosServices with ChangeNotifier {
  ...
  void getPedidosEnRutaProveedor({User user}) async {
    // OBTENEROS LOS DATOS DEL USER REGISTRADO
    final url =
        '$_api/pedidos?idUser.id_eq=${user.id}&idEvento=$STATUS_EN_RUTA&idEstadoLiquidacion=$STATUS_PENDIENTE';
  ...
  void getPedidosPendientesProveedor({User user}) async {
    final url =
        '$_api/pedidos?idUser.id_eq=${user.id}&idEvento=$STATUS_ENTREGADO&idEstadoLiquidacion=$STATUS_PENDIENTE';
    final resp = await http.get(url);
    final pedidosResponse = pedidosFromJson(resp.body);
  ...
  void getHistorialPedidosProveedor({User user}) async {
    final url =
        '$_api/pedidos?idUser.id_eq=${user.id}&idEvento=$STATUS_ENTREGADO&idEstadoLiquidacion=$STATUS_EFECTIVO';
    final resp = await http.get(url);
    final pedidosResponse = pedidosFromJson(resp.body);
